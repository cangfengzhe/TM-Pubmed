...
#主函数，根据所需关键词获取对应的pmid， 输入文章的pmid（多个以逗号隔开）可以得到文章的题目，摘要，杂志名称 ，摘要
#load('stroke.rdata');
library(ff)
library(ffbase)
source('regexp.R')
...
source('pubmedText.R')
pmid_list = downloadPmid('str'); #根据所需关键词获取对应的pmid
pmid='';
title='';
abstract='';
journal='';
...
n=0
error0=NA
for(ii in 1:(as.integer(length(pmids_list)/k)+1)){
  tryCatch({
    idList<-paste(pmids_list[((ii-1)*k+1):(ii*k)],collapse=','); # 多个pmid以“，”隔开
